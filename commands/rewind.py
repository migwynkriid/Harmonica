        ...
        try:
            ...
            if seconds <= 0:
                await ctx.send(embed=create_embed(
                    "Error", 
                    "Rewind amount must be greater than 0 seconds!", 
                    color=0xe74c3c, 
                    ctx=ctx
                ))
                return
            ...
            success, message, new_position = await seek_audio(ctx, music_bot, seconds, direction="rewind")
            ...
            embed = create_embed(
                "⏪ Rewind",
                f"Rewound {seconds} seconds to {message}\n[{current_song['title']}]({current_song['url']})",
                color=0x3498db,
                ctx=ctx
            )
            ...
            await ctx.send(embed=create_embed(
                "Error", 
                f"An error occurred while rewinding: {str(e)}", 
                color=0xe74c3c, 
                ctx=ctx
            ))
